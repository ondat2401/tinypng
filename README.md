...
## Cách lấy API key riêng (miễn phí)

Mỗi tài khoản TinyPNG được **500 lượt nén/tháng miễn phí**. Khi key dùng chung hết quota, bạn có thể tự tạo key riêng:

1. Mở trang đăng ký key: 👉 **https://tinypng.com/developers**
2. Nhập **tên** và **email** của bạn rồi bấm **"Get your API key"**.
3. Mở email TinyPNG gửi tới, bấm link xác nhận để vào **Dashboard**.
4. Trong Dashboard, copy chuỗi **API key** (dạng `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`).
5. Mỗi email tạo được 1 key (500 lượt/tháng). Cần thêm quota thì đăng ký bằng email khác.

## Tự thêm key khi hết quota

Không cần sửa code hay file cấu hình — làm ngay trên web:

1. Ở thanh **Quota** phía trên, bấm nút **"Manage keys"**.
2. Dán API key vừa lấy vào ô **"Paste a new API key…"** rồi bấm **Add**.
3. Key mới được lưu trong **trình duyệt của bạn** (localStorage) và tự động được chọn để dùng.
4. Khi một key **hết quota**, web **tự động chuyển sang key kế tiếp** trong danh sách rồi nén tiếp các ảnh còn lại. Thêm nhiều key từ nhiều email khác nhau để **cộng dồn quota**.

> Mẹo: giới hạn 500 lượt/tháng tính theo **từng tài khoản** TinyPNG, nên dùng key từ các email khác nhau sẽ tăng tổng số lượt nén.
